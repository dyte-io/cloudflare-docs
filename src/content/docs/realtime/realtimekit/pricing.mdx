...
Whether a participant is an audio-only participant or an audio/video participant is determined by the `Meeting Type` of the [preset](/realtime/realtimekit/concepts/preset/).