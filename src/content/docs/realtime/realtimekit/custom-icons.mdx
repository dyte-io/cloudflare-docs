...
<head>
  <script type="module">
    import { provideRtkDesignSystem, extendConfig } from 'https://cdn.jsdelivr.net/npm/@cloudflare/realtimekit-ui/dist/esm/index.js';
  </script>
</head>
```
...
1. Create a meeting room using the [Create Meeting API](/realtime/api/)
2. Generate an `authToken` using the [Add Participant API](/realtime/api/). An `authToken` is a unique token that is used to identify a user in the meeting
...
- [Render Default Meeting UI](/realtime/realtimekit/render-default-meeting-ui/) - Complete meeting experience out of the box
