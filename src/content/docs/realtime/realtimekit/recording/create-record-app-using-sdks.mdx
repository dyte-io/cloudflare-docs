...
The custom recording SDKs are used on top of the [UI kit](/react-ui-kit) or [Vanilla JS](/ui-kit). The `RealtimeKitRecording` class is used for managing the recording functionality within the SDK.
...
Once you've created the app, deploy it using a platform like [Vercel](https://vercel.com/docs/concepts/get-started). Make sure to note the URL where you have deployed the app, as you will have to enter this URL in RealtimeKit's recording API.
