...
:::note
This guide assumes that you are already familiar with [initializing the RealtimeKit SDK](/realtime/realtimekit/core/initialization).
:::
...
export default function App() {
    ...
    useEffect(() => {
        if(!media){
            const mediaFromSDK = RealtimeKitClient.initMedia({
              video : true,
              audio: true,
            });
