...
:::note[Prerequisites]
This page builds upon the [Initialize SDK](/realtime/realtimekit/initialize-sdk/) and [Render Default Meeting UI](/realtime/realtimekit/render-default-meeting-ui/) guides. Make sure you've read those first.
...
:::note
These are **UI Kit internal states** for managing the interface. For meeting data like participants, active speaker, or recording status, use the [Core SDK's meeting object](/realtime/realtimekit/meeting-object-explained/) directly.
:::
...
flowchart LR
    ...
    Toggle["👤 RtkParticipantsToggle<br/>(User clicks)"]
...
function App() {
    ...
    useEffect(() => {
        ...
        return () => {
            meetingRef.current?.removeEventListener('rtkStatesUpdate', handleStatesUpdate);
        };
    }, [meetingRef.current]);
