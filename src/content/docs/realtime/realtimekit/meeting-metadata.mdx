...
function SocketConnectionMonitor() {
  ...
  useEffect(() => {
    if (meeting) {
      const handleSocketConnection = ({ state, reconnectionAttempt, reconnected }) => {
        // state - 'connected' | 'disconnected' | 'reconnecting' | 'failed'
...
- [Meeting Object Explained](/realtime/realtimekit/meeting-object-explained/) - Comprehensive meeting object reference
- [Peer Lifecycle](/realtime/realtimekit/peer-lifecycle/) - Understanding meeting states and transitions