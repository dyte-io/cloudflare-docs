...
The breakout rooms feature is currently in beta, which means it is still being tested and evaluated, and may undergo some changes. 
...
Each breakout session is a separate session. Each breakout session's recording is stored and managed separately, just like any other RealtimeKit meeting. For more information, refer to [Recording](/realtime/realtimekit/recording/).
...
Create a RealtimeKit meeting using the [Create Meeting API](/realtime/api/#create-meeting). This API returns a unique identifier for your meeting.

### Step 4: Add participants

After creating the meeting, add each participant using the [Add Participant API](/realtime/api/#add-participant). The `presetName` created earlier must be passed in the body of the Add Participant API request.
...
- [UI Kit Components Library](/realtime/realtimekit/components-library/) - Browse available components
- [UI Kit States](/realtime/realtimekit/ui-kit-states/) - Learn how components synchronize
- [Build Your Own UI](/realtime/realtimekit/build-your-own-ui/) - Create custom meeting interfaces