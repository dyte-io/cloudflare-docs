...
function WaitingRoomStatus() {
  ...
  return isWaitlisted ? (
    <div>Waiting for host approval...</div>
  ) : null;
}
```
...
function RejectionStatus() {
  ...
  return rejected ? (
    <div>Your entry was rejected by the host</div>
  ) : null;
}
```
...
Hosts can manage waiting room requests using participant management methods. See [Remote Participants](/realtime/realtimekit/remote-participants/) for details on:
